...
function BlogSideCard({ thumbnail, title, content, author, onBack }) {
  return (
    <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm cursor-pointer hover:shadow-md transition-shadow" onClick={onBack}>
      {/* Thumbnail */}
      {thumbnail ? (
        ...
        <div className="w-full h-36 bg-gradient-to-br from-indigo-800 to-indigo-600 flex items-center justify-center">
          <span className="text-white font-black text-lg tracking-wide">React JS</span>
        </div>
      )}
      {/* Body */}
      <div className="p-4">
        ...
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-indigo-100 overflow-hidden flex-shrink-0 border border-indigo-200">
            <img
              src="/avatar.jpg"
              alt={author}
              ...
              onError={(e) => {
                ...
                e.target.parentElement.innerHTML = `<span class="w-full h-full flex items-center justify-center text-indigo-600 font-bold text-xs">${(author ?? "A")[0]}</span>`
              }}
            />
          </div>
          <div>
  ...
  return (
    <div className="min-h-screen bg-white">
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 py-3.5 px-6 md:px-20">
        <button onClick={onBack} className="inline-flex items-center gap-2 bg-indigo-900 text-white text-sm font-bold px-5 py-2 rounded-full shadow-lg shadow-indigo-200">
          ← Back
        </button>
      </nav>
    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-16 py-16 grid grid-cols-1 md:grid-cols-[1fr_300px] gap-14 items-start">
        ...
        <article>
          ...
          {/* Content */}
          <p className="text-sm md:text-base text-gray-600 leading-relaxed mb-4 whitespace-pre-wrap">
            {content}
          </p>
