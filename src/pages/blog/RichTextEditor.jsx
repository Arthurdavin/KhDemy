// RichTextEditor.jsx
// Install deps: npm install @tiptap/react @tiptap/pm @tiptap/starter-kit @tiptap/extension-placeholder @tiptap/extension-underline @tiptap/extension-text-align @tiptap/extension-link @tiptap/extension-image
...
const ToolbarBtn = ({ onClick, active, disabled, title, children }) => (
  <button
    ...
    disabled={disabled}
    title={title}
    className={`p-1.5 rounded-md transition-all text-gray-500 hover:text-gray-800 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed ${
...
export default function RichTextEditor({ value, onChange, placeholder = "Write your blog content here…" }) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-sm max-w-none min-h-[220px] px-4 py-3 text-gray-800 text-sm leading-relaxed outline-none",
      },
    },
  });
  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden bg-white focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
      {/* ── Toolbar ── */}
      ...
      {/* ── Word count ── */}
      <div className="flex justify-end px-3 py-1 border-t border-gray-50 bg-gray-50/50">
      ...
      <style>{`
        ...
        .tiptap h1 { font-size: 1.5rem; font-weight: 800; margin: 0.75rem 0 0.5rem; color: #1f2937; }
        .tiptap h2 { font-size: 1.25rem; font-weight: 700; margin: 0.65rem 0 0.4rem; color: #1f2937; }
        .tiptap h3 { font-size: 1.1rem;  font-weight: 600; margin: 0.5rem 0 0.35rem; color: #1f2937; }
        .tiptap p  { margin: 0.25rem 0; }
        .tiptap ul, .tiptap ol { padding-left: 1.5rem; margin: 0.4rem 0; }
        .tiptap li { margin: 0.15rem 0; }
        .tiptap blockquote { border-left: 3px solid #6366f1; padding-left: 1rem; margin: 0.5rem 0; color: #6b7280; font-style: italic; }
        .tiptap code { background: #f3f4f6; padding: 0.1rem 0.35rem; border-radius: 4px; font-size: 0.8rem; color: #6366f1; }
        .tiptap hr { border: none; border-top: 1px solid #e5e7eb; margin: 0.75rem 0; }
        .tiptap pre { background: #1f2937; color: #f9fafb; padding: 1rem; border-radius: 8px; overflow-x: auto; font-size: 0.8rem; }
      `}</style>
    </div>
  );
}